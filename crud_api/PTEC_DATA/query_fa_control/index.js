...
const FA_Control_New_Assets = async (req) => {
  ...
  try {
    ...
    const fetch_assets = await pool.request()
      ...
      .query(`exec ${config.PTEC.object_test_ops.sql.database}.dbo.FA_Control_New_Assets @UserCode, @Code, @Name, @BranchID, @Details , @SerialNo, @Price, @Create_Date, @bac_type`);
    //sql.close()
    return fetch_assets.recordset;
  } catch (error) {
    //sql.close()
...
const FA_Control_New_Assets_Xlsx = async (req) => {
  ...
  try {
    ...
    const fetch_assets = await pool.request()
      ...
      .query(`exec ${config.PTEC.object_test_ops.sql.database}.dbo.[FA_Control_Upload_Assets_Xlsx] @Code, @Name, @BranchID , @OwnerCode, @SerialNo, @Price, @CreateDate, @CreateBy, @Position, @Details, @bac_type, @key`);
    //sql.close()
    return fetch_assets.recordset;
  } catch (error) {
    //sql.close()
