...
const OPS_Mobile_List_Vender = async (branchIDparam) => {
  ...
  try {
    ...
    const assetslist = await pool.request()
      .query(`exec ${config.PTEC.object_test_ops.database}.[dbo].[OPS_Mobile_List_Vender]`);
    //sql.close()
...
const STrack_Registation = async (req) => {
  ...
  try {
    ...
    const assetslist = await pool.request()
      ...
      .query(`exec ${config.PTEC.object_test_ops.database}.[dbo].[STrack_Registation] @userCode, @email, @venderCode, @name, @lastName, @phoneNumber`);
    //sql.close()
    return assetslist.recordset;
  } catch (error) {
    //sql.close()
...
const STrack_CheckVenderID = async (req) => {
  ...
  try {
    ...
    const assetslist = await pool.request()
      ...
      .query(`exec ${config.PTEC.object_test_ops.database}.[dbo].[STrack_CheckVenderID] @userid`);
    //sql.close()
    return assetslist.recordset;
  } catch (error) {
    //sql.close()
...
const STrack_callMessages = async (req, res) => {
  ...
  try {
    ...
    const assetslist = await pool.request()
      ...
      .query(`exec ${config.PTEC.object_test_ops.database}.[dbo].[STrack_callMessages] @message, @userid_line`);
    //sql.close()
    return assetslist.recordset;
  } catch (error) {
    //sql.close()
...
const STrack_responseFlex_AfterInsert = async (req, res) => {
  ...
  try {
    ...
    const assetslist = await pool.request()
      ...
      .query(`exec ${config.PTEC.object_test_ops.database}.[dbo].[STrack_Send_AcceptJobs] @jobcode, @dtlid, @usercode`);
    //sql.close()
    return assetslist.recordset;
  } catch (error) {
    //sql.close()
