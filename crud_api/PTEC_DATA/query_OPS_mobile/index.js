...
const STrack_Registation = async (req) => {
  ...
  try {
    let pool = await sql.connect(config.PTEC.object_test_ops.sql);
    const assetslist = await pool.request()
      .input('userCode', sql.VarChar(100), req.userid)
      .input('email', sql.VarChar(100), req.email)
      .input('venderCode', sql.VarChar(100), req.venderCode)
      .input('name', sql.VarChar(100), req.name)
      .input('lastname', sql.VarChar(100), req.lastname)
      .input('phoneNumber', sql.VarChar(100), req.phoneNumber)
...
const STrack_CheckVenderID = async (req) => {
  ...
  try {
    let pool = await sql.connect(config.PTEC.object_test_ops.sql);
    const assetslist = await pool.request()
      .input('userid', sql.VarChar(100), req)
